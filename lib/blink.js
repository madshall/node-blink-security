...
module.exports = class Blink {
      ...
      promises.push(
        new Promise((resolve, reject) => {
          if (!this._auth_header) {
            return reject(
              new BlinkException('Authentication token must be set')
            );
          }
      ...
      request(
        ...
        (err, response, body) => {
          if (err || _statusCodeIsError(response)) {
            return reject(
              new BlinkException(
                `Can't retrieve connected clients`,
                this.debug ? response : null
              )
            );
          }
  ...
  setArmed(value = true, networkIds = []) {
    const promises = [];
    const networksToArm = networkIds.length
      ? networkIds
      : this.networks.map((_) => _.id);
          ...
          request.post(
            ...
            (err, response, body) => {
              if (err || _statusCodeIsError(response)) {
                return reject(
                  new BlinkException(
                    `Can't ${state} the network: ${networkId}`,
                    this.debug ? response : null
                  )
                );
              }
      ...
      request(
        {
          url: `${
            this.urls.video_url
          }?since=${since.toISOString()}&page=${page}`,
          headers: this._auth_header,
          json: true,
        },
        (err, response, body) => {
  ...
  setupSystem(name_or_id) {
    if (
      !((this._username && this._password) || (this._token && this._region_id))
    ) {
  ...
  _getAuthToken(repeats = 0) {
    return new Promise((resolve, reject) => {
      if (typeof this._username != 'string') {
        return reject(
          new BlinkAuthenticationException('Username must be a string')
        );
      }
      if (typeof this._password != 'string') {
        return reject(
          new BlinkAuthenticationException('Password must be a string')
        );
      }
      if (typeof this._device_id != 'string') {
        return reject(
          new BlinkAuthenticationException('Device ID must be a string')
        );
      }
      ...
      request.post(
        ...
        (err, response, body) => {
          if (err || _statusCodeIsError(response)) {
            return reject(
              new BlinkAuthenticationException(
                `Authentication problem: ${body.message}`
              )
            );
          } else {
            if ((body.client || {}).verification_required) {
              if (!this.auth_2FA) {
                if (repeats === 1)
                  return reject(
                    new BlinkAuthenticationException(
                      `Authentication problem: verification timeout`
                    )
                  );
                return new Promise((resolve, reject) => {
                  setTimeout(() => {
              ...
              return rl.question(
                `Enter the verification code sent to ${this._username}: `,
                (pin) => {
                  request.post(
                    {
  ...
  getIDs(name_or_id) {
    ...
    return new Promise((resolve, reject) => {
      if (!this._auth_header) {
        return reject(
          new BlinkException(
            'You have to be authenticated before calling this method'
          )
        );
      }
      request(
        ...
        (err, response, body) => {
          if (err || _statusCodeIsError(response)) {
            ...
            if (typeof name_or_id != 'undefined') {
              ...
              if (!network) {
                return reject(
                  new BlinkException('No network found for ' + name_or_id)
                );
              }
            } else {
