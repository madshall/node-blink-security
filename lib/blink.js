        ...
        if (err || _statusCodeIsError(response)) {
          ...
          if ((body.client || {}).verification_required) {
            ...
            return rl.question(`Enter the verification code sent to ${this._username}: `, pin => {
              this._region_id = body.region.tier;
              this._region = body.region.code;
              this._token = body.authtoken.authtoken;

              const urls = new BlinkURLHandler(body.account.id, body.region.tier);
              headers['token-auth'] = body.authtoken.authtoken;

              console.log(`url: ${urls.base_url}/api/v4/account/${body.account.id}/client/${body.client.id}/pin/verify`);
              console.log('headers: ' + JSON.stringify(headers));
              console.log('pin: ' + pin);

              request.post({
                url: `${urls.base_url}/api/v4/account/${body.account.id}/client/${body.client.id}/pin/verify`,
                json: true,
                headers: headers,
                body: {
                  pin: `${pin}`,
